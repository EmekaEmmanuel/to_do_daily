      ...
      for (let i = 0; i < storingparam.length; i += 1) {
        eachList
          += `<article key=${storingparam[i].index} class=" todo_item">
            ...
            <input ${storingparam[i].completed === true ? 'checked' : ''} value=${storingparam[i].completed} checkbox_index=${i} class="border_none checkbox_tag checkbox_${i}" type="checkbox" required>
